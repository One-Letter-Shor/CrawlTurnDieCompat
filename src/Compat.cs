...
public static class Compat
{
    ...
    public static void CheckMods()
    {
        ...
        if (Logger.CanLogUsingType) Logger.Info($"Using {ActiveCompats} as the {nameof(Compats)}.");
    }
    
    private static void OnActiveGameModeChanged(RainWorldGame game, ProcessManager? processManager)
    {
        if (Logger.CanLogUsingType) Logger.Info($"Using {ActiveGameMode} as the {nameof(GameMode)}.");
